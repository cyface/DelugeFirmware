...
```shortcut
hold(SHIFT) + press(KIT), turn(SELECT), hold(SELECT), menu("Load all"), press(SELECT)
```
...
```shortcut
hold(SHIFT), press(KIT), turn(SELECT), hold(SELECT), menu("Slice"), press(SELECT), menu("Num. slices"), press(SELECT)
```
...
```shortcut
hold(AUDITION) + press(GRID), turn(SELECT), menu("Choke"), press(SELECT)
```
