...
- <Badge text="c1.4" variant="tip" /> A new `Drum` oscillator type plays
  sample-free 808 / 909-style kick, snare and hi-hat models ported from Mutable
  Instruments Plaits, each with Tone / Decay / Drive-Snappy-Noise macros that
  are automatable and patchable. Velocity drives the models' accent. Works in
  kits and synth clips. - A community setting has to be activated to select the
  new oscillator type. See the separate document for details.
